...
var appKey = "0BA3CDB8-5152-BCDB-BFBD-1D9520D5917B";
var appSecret = "c488d12e8905304844eed1a508f937a3";
var privateKey = "277426D8FC86325EFD885BA1F6A76280";
...
var AgentLayer = cc.Layer.extend({
    ...
    ctor:function () {
        ...
        cc.log("appKey:"+appSecret);
        cc.log("appKey:"+privateKey);
        ...
        cc.log("share_plugin:"+share_plugin);
        if (share_plugin != null)
            share_plugin.setResultListener(this.onShareResult, this);

        cc.log("analytics_plugin:"+analytics_plugin);
        if (analytics_plugin != null)
            cc.log("analytics_plugin:"+analytics_plugin);
            // analytics_plugin.startSession();
        cc.log("push_plugin:"+push_plugin);
        if (push_plugin != null)
            cc.log("startPush.");
        ...
        this.base_menu = ["User System", "IAP System", "Share System", "Ads System", "Social System", "Push System", "Other Oper"];
        ...
        this.sec_menu = [
                    ...
                    ["close Push", "set Alias", "del Alias", "set Tags", "del Tags", "startPush"],
                    ["Custom Param", "channelID", "UserId", "OrderId", "islogined"],
                    ];
    ...
    onClick:function(item){
        ...
        else if (tag < menu_lv.other){
            this.onOtherAction(tag - menu_lv.push);
        }
    },
    ...
    onUserAction:function(idx){
        ...
        switch(idx){
            case user_operation.login:{
                    user_plugin.login("server_id", "server_ip");
                    if (analytics_plugin) 
            ...
            case user_operation.submitLoginGameRole:
                    ...
                    cc.log("show toolbar 111");
                    this.onUserAction(user_operation.showToolbar);
                    cc.log("show toolbar 222");
                    this.onUserAction(user_operation.hideToolbar);
                    cc.log("show toolbar 333");
                break;
        }
    },
        ...
        switch(idx){
            case ads_operation.showAds:
                if ( ads_plugin.isSupportFunction("AD_TYPE_FULLSCREEN") )
                    ...
                    if (analytics_plugin) 
                        analytics_plugin.logEvent("showAds")
                break;
    ...
    onPushAction:function(idx){
        ...
        switch(idx){
            ...
            case push_operation.setTags:
                    var tags = ["easy","fast","qwe"];
                    cc.log("tags:"+tags);
                    for(var item in tags){
                        cc.log("item:"+item);
                    }
                    push_plugin.setTags(tags);
                break;
            case push_operation.delTags:
                    push_plugin.delTags(["easy","qwe"]);
                break;
            case push_operation.startPush:
                    cc.log("click startPush");
                    push_plugin.startPush();
                break;
        }
    },
    onOtherAction:function(idx){
        switch(idx){
            case other_operation.CustomParam:
                var param = agent.getCustomParam();
                cc.log("customParam:"+param);
            break;
            case other_operation.ChannelID:
                var param = agent.getChannelId();
                cc.log("ChannelID:"+param);
            break;
            case other_operation.UserID:
                var param = user_plugin.getUserID();
                cc.log("UserID:"+param);
            break;
            case other_operation.OrderID:
                for(var key in iap_plugin){
                    var iap_plu = iap_plugin[key];
                    var param = iap_plu.getOrderId();
                    cc.log("orderID:"+param);
                }
            break;
            case other_operation.IsLogin:
                var param = user_plugin.isLogined();
                cc.log("isLogined:"+param);
            break;
        }
    },
